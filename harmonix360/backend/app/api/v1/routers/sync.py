...
async def pull(
    entity_types: str = Query(..., description="Comma-separated syncable entity types, e.g. 'note,asset'"),
    ...
    return await service.pull(since=since, entity_types=requested, limit=limit)
...
async def push(
    ...
    service = SyncService(db)
    return await service.push(mutations=dto.mutations, actor_key=current_user.email)

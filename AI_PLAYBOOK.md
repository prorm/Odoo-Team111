...
This is the governing constitution for the Harmonix360 Platform. Multiple human developers and AI models operate concurrently in this repository. Strict adherence to this playbook ensures our code compiles, integrates, and functions without conflicts.
...
We are building Harmonix360, a modern modular enterprise platform. The architecture is modular and immutable at its core, but highly flexible at its domain boundaries. 
...
- **WHY:** Multi-tenancy and auditability are non-negotiable platform requirements.
...
Focus on integration tests alongside unit tests. Test the critical path (API -> Service -> DB).
...
- **WHY:** Cross-tenant data leaks cause severe security vulnerabilities.
...
Wrap new experimental features in flags.
- **GOOD:** `if (flags.isEnabled('AI_ASSET_TAGGING')) { ... }`
- **BAD:** Hardcoding experimental, unstable logic into the main branch execution path.
- **WHY:** If a feature fails during verification, it can be disabled via configuration rather than redeploying.
...
## 40. Rapid Development Best Practices
- **Prioritize the "Happy Path":** Ensure the core flow works reliably before adding edge-case validations.
...
- **Speed Over Purity (but follow boundaries):** If prototyping, do it inside the `/modules/{domain}` folder so the core architecture remains clean.
